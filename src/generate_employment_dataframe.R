...
acs$geography <- gsub(pattern = "Congressional District ([0-9]+) \\(116th Congress\\), California",
                           -                          replacement = "\\1",
                           -                          acs$geography)
acs$geography = toupper(acs$geography)
...
acs <- acs %>% mutate(armed_forces_percent = (acs$armed_forces_employed / acs$armed_forces_employed[112]),
...
mili_county <- acs %>% filter(is.na(as.numeric(acs$geography))) %>%
  filter(!(geography == "CALIFORNIA"))
mili_county <- mili_county %>%
  rename(county = geography, mili_emp = military_personnel)

mili_district <- acs %>% filter(!is.na(as.numeric(acs$geography)))
